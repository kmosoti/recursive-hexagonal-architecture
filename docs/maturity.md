...
| Ambient-effect deny list; `no_std` core option | §6.8 | `xtask/templates/core-clippy.toml` (89 entries), `xtask/src/clippy_template.rs` (rule `effect.core_clippy_template`), `xtask/tests/clippy_corpus.rs` | xtask 0.1.0 (CHG-001, CHG-001.1, CHG-001.2) | **I** for the deny list: every one of the template's 89 entries fails the build where a fixture crate uses it, the configuration discovery rule is recorded (ADR-0002), and so is what switches the list off. Not V yet: no product crate uses the template, the rule is not enforced in L0 until CHG-003, and the attribute escape of ADR-0002 rule 5 is open (V proposal planned at CHG-006). `no_std` option: S (experiment E1) | `evidence/w1-clippy/` (21 experiment records); L0: `evidence/CHG-001/20260919T222005Z-bcaf3090691d.json`; CI: `evidence/ci/35473050927.json` | generated corpus fixtures only, crate level: direct uses in fixture crates on x86_64 Linux; not transitive calls, not macro-generated calls, not paths missing from the list, and not a crate that allows the lint (ADR-0002); the rule is tested on temporary directories and is not yet run by any L0 command | Clippy 0.1.98; local WSL2 and GitHub `ubuntu24` (the corpus runs in `L0.nextest` in both) | S (spec v0.10) |
