...
**Re-review of f73f736: APPROVE; two P3s repaired before merge.** CHG-008.toml now lists what it accepts (`covers = CHG-008 to CHG-012`), because a merged acceptance record cannot be edited later. The two lessons-table cells for L1 and L3 now say per stage PR. Both CI evidence entries now carry their sha256, as CHG-007's do. The reviewer also showed that `rha lint` accepts an acceptance record with an all-zero tree, a wrong digest or flipped predicates. That is recorded as a follow-up; these records were verified by recomputation, not by the lint.
