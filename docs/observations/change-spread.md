...
| Stage | Original plan prediction | Predeclared production set | Predeclared additional changes | Start | End / observed |
| --- | --- | --- | --- | --- | --- |
| Callouts (CHG-012) | document + adapter-html | adapter-html only: the callout title paragraph and stylesheet rules | The registered render grader and golden HTML (a separate session, before code); records and generated docs. No change in document, site, adapter-json or app-cli: the kind is already parsed (Phase 1) and JSON output is unchanged. The plan's document part is refuted before code (decision callouts-html-title). | This contract and prediction commit, before corpus and code | pending |
