...
- **M6, review.** When the packet is ready, get two reviews of the same head, as `.rha/decisions.toml` DP-5.4 sets them: an independent Opus 5.5 reviewer, and a GPT-6 validator that checks conformance to the registered contracts and never generates code or cases. Graded cases come from a separate session (§2.4 rules 2 and 5); the Executor never grades its own. Findings are judged by AGENTS.md. Every confirmed finding gets a determination in the change record and a repair. Re-reviews see only the repair diff. After three rounds, remaining disagreements go to Kennedy. Both verdicts are posted on the PR; the integrator merges only when both approve that head.
  - **Revision 3:** the second-family review may be an agy teamwork run used only for verification: product code, docs and records are frozen in its jail, and it writes probes and a report only. It counts as the second family while GPT-6 is unavailable (decision merge-rule-gpt6-unavailable), and as a third reviewer after that. Implementation by agy uses Boost or a single agent, never teamwork (Kennedy, 2026-09-25).
