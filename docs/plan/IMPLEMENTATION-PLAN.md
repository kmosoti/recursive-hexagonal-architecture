...
> **Provenance.** Revision 1 was planned on 2026-09-19 with Claude Fable 5.1 under Kennedy Mosoti's direction and approved the same day; CHG-001.1 made the Executor model-agnostic. **Revision 2 (PLAN-2, 2026-09-22)** was redeveloped with Claude Opus 5.5 at Kennedy's direction after W0 to W4 closed: "take all lessons so far and redevelop the plan, with larger work items leveraging the extremely capable Opus 5.5 stochastic generation". It keeps the subject, the architecture, the xtask design, the corpus and the record shapes (§3 to §7), and replaces the operating method, the work breakdown, the decision catalogue and the risk table. The merge of PLAN-2 is Kennedy's approval of revision 2 (spec §11.7.1: the task brief owns authorized intent; it grants no authority beyond permissions and `.rha/policy.toml`). **Revision 3 (PLAN-4, 2026-09-25)** follows a study of the program's own history (stage delivery, lessons forward, revision differentials, stage size): M2's stage-delivery rule and M11 to M13; where revision 2 text says a packet is one PR, revision 3 wins.

## How to start a packet

Open a coding-agent session in this directory. The recommended assignment (§2.2) is Claude Opus 5.5 in Claude Code as Executor, with the largest context available; effort comes from the session's configuration. Reviewers and their models are set by `.rha/decisions.toml` DP-5.4. Records stay model-agnostic: the actor is `agent:executor` and provenance names the model. Paste:

```text
You are the Executor for packet P-<X> of docs/plan/IMPLEMENTATION-PLAN.md (revision 3), in `agent` mode under ECC-Solo. Kennedy Mosoti is Planner, Integrator and Acceptor.
...
2. Follow §2 exactly. The packet is one branch, built as the stages §8.1 lists, each stage a commit series ending green; each stage that delivers a feature, measurement or harness is merged as its own PR with CI (M2). The first commit of each PR is the previous merged item's acceptance record.
3. At every gate §8.1 names, take the recommended option under Kennedy's delegation of 2026-09-22 and record it on the ledger with its decision id. Held-out material and any use of gpt-6-astra stay Kennedy's to decide. Never touch a protected surface without an approval quoted in the task record.
...
5. When a stage is green, run `cargo xtask scope --task <id>` and one local L0 record for that stage PR, then get the two reviews DP-5.4 requires (M6), both approving the same head. Answer every finding by consequence (AGENTS.md), then open the stage PR and stop. Do not merge.
```
...
| --- | --- | --- | --- |
| L1 | Recording is the main source of churn. A lane record, then a docs regeneration, then a CI record, each committed, each triggering another CI run and review | 46 to 61 of 132 commits, depending on definition (above); PR 2 had 38 commits | M5: one local record per settled stage PR (revision 3). A PR-head CI record is never committed; the next acceptance record commits `main`'s |
...
| L3 | Automated review on every push multiplied churn. One review at "ready" plus focused re-reviews converged in three rounds and found real defects each time | PRs 4 to 6: 17 findings; PR 14: 7 findings, 3 rounds | M6: one local review when the stage PR is ready (revision 3), re-reviews only of the repair diff, at most three rounds, by the AGENTS.md rules |
...
### 2.2 Operating method (revision 2, amended by revision 3)
...
- **M2, the packet.** A packet is one branch, `chg/<first-id>-<slug>`, merged stage by stage (revision 3, below). It covers one or more of revision 1's CHG ids, which keep their numbers because records and code cite them. It has one task record with `covers = [...]` and one change record in the §11.7.5 template. It is built as **stages**. A stage is a commit series that ends with the tree green under `cargo nextest run` and `cargo xtask docs --check`. A stage that fails its gate is repaired inside that stage (§11.7.10 repair attempts), before its PR merges. The first commit is the previous item's acceptance record. The merge is the acceptance.
  - **Revision 3 (PLAN-4), stage delivery.** A stage that delivers a feature, a measurement or a harness is merged on its own: one PR per stage, from the packet branch or a stage branch cut from it, with CI on the PR and the merge as that stage's acceptance. The packet keeps one task record and one change record, and change-spread stays measured per stage by commit range. Reason: the study `studies/2026-09-rha-stage-division` found that P-D's first six stages ran 48 commits without a PR or a CI run, because `ci.yml` runs only on pull requests and on main. Learning happened per stage, but delivery happened per packet.
...
- **M5, evidence discipline.** Regenerate docs, then run the lane, then commit. Take **one** local L0 record per stage PR, on the settled clean head, plus the packet's own harness records (H4, H5, bench). Keep failed and superseded records; never rewrite them. Do not commit PR-head CI records; the CI run is visible on the PR. The next acceptance record (the first commit of the next PR, stage or packet) downloads and commits the record of the run on `main` at the merge.
- **M6, review.** When a stage PR is ready, get two reviews of the same head, as `.rha/decisions.toml` DP-5.4 sets them: an independent Opus 5.5 reviewer, and a GPT-6 validator that checks conformance to the registered contracts and never generates code or cases. Graded cases come from a separate session (§2.4 rules 2 and 5); the Executor never grades its own. Findings are judged by AGENTS.md. Every confirmed finding gets a determination in the change record and a repair. Re-reviews see only the repair diff. After three rounds, remaining disagreements go to Kennedy. Both verdicts are posted on the PR; the integrator merges only when both approve that head.
  - **Revision 3:** the second-family review may be an agy teamwork run used only for verification: product code, docs and records are frozen in its jail, and it writes probes and a report only. Its standing is decision agy-verification-second-family; which agy modes implement is decision agy-teamwork-verification-only (both in `.rha/tasks/CHG-008-growth.toml`).
...
- **M11, lessons carried forward (revision 3).** A stage's close record ends with a list headed "For the next stage". The next stage's contract or prediction commit cites each item and applies it, or rejects it with a reason. A lesson that a later stage has to rediscover is recorded as a defect of the earlier close. Reason: CHG-009's pure-fixture-inputs decision did not reach CHG-010's grader, and the implementing agent found the problem again (4be1a7d).
- **M12, a revision differential at contract time (revision 3).** Every stage that must leave the rest of the product unchanged registers, at its contract commit and before code, a differential against the parent revision. The differential covers the real docs tree plus generated inputs, has a stated tolerance (for example, only lines of a named form may differ), and includes a negative control showing it can fail. Hand-picked goldens remain, but they are secondary. Reason: in CHG-010 and CHG-011, on average 3.5 P1/P2 defects per stage passed every registered grader and were found only by independent review. The CHG-011 `![` defect was invisible to all 44 goldens but visible to a differential.
- **M13, stage size (revision 3).** A stage whose registration exceeds 200 cases or 3 packages, or whose prediction spans more than 3 production crates, is split before code into stages that each satisfy M12. A stage that needs more than 3 repair attempts is reported as wrongly sized in its close record. Reason: transclusion needed 11 repair attempts and 507 registered cases in one stage.
...
Revision 1 cut the work into items a session could finish in one context. The failures were not implementation failures. They were recording and review loops around small PRs (L1, L3). A capable Executor with a large context can carry a whole milestone, provided three conditions hold. The stages are gated, so a wrong turn is caught within one stage. The scope guard catches improvisation mechanically (L4). The evidence is taken once per merged stage, not per commit (L1; revision 3 moved it from per packet to per stage). Each packet in §8.1 ends at a milestone Kennedy can accept in one review, with a review index to read it by.
...
### 8.1 Packets (revision 2; P-D and later stages delivered per M2 of revision 3)
...
**P-D. Growth under measurement (CHG-008 to CHG-014).** The W8, W9 and W10 briefs. Under revision 3 (M2), the stages completed so far (stage 0, stage 1, and stage 2 features 1 to 4) merge as one catch-up PR, because they were built before M2 changed. Stage 2 feature 5 and stage 3 each follow as their own PR. `docs/observations/change-spread.md` reports, as its headline, how accurate the plan's original predictions were (plan prediction against observed crates), separately from the refined predictions made before code after probing. By 2026-09-25 the original prediction was exact in 2 of 5 stages, and the refined one in 5 of 5. The refined column measures how good the pre-code probing was, not the architecture. Each feature is its own stage and commit range. The expected touched set is committed at the start of the stage, and the observed set is recorded at its end, so change-spread is measured per stage (`git diff --numstat <stage-start>..<stage-end>`), not per PR. Stage 3 waits for DP-2.1 if P-C has not recorded it. Mutation survivors are explained by a generated question per survivor, and Kennedy adjudicates (DP-2.2).
...
| Recording churn returns | One L0 record per stage PR (M5); no PR-head CI records; docs, then lane, then commit (M5) |
