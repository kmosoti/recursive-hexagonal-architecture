...
5. When the packet is green, run `cargo xtask scope --task <id>` and one local L0 record, then get the two reviews DP-5.4 requires: an Opus 5.5 reviewer and a GPT-6 validator, both approving the same head. Answer every finding by consequence (AGENTS.md), then open the PR and stop. Do not merge.
```
