...
struct ChangingAssetsRenderer {
    counter: Cell<u8>,
}

impl PageRenderer for ChangingAssetsRenderer {
    fn render(&self, page: &PageModel) -> Rendered {
        GoodEchoRenderer.render(page)
    }
    ...
    fn assets(&self) -> Vec<Rendered> {
        self.counter.set(self.counter.get().wrapping_add(1));
        vec![Rendered {
            path: RelPath::new("_assets/changing.css").expect("valid"),
            bytes: vec![self.counter.get()],
        }]
    }
}
...
fn renderer_contract_reports_nondeterminism_and_panics() {
    ...
    assert!(!panicking.is_empty(), "{panicking:?}");
    assert!(
        panicking
            .iter()
...
fn renderer_contract_detects_a_renderer_that_drops_toc() {
    ...
    assert!(
        violations
            .iter()
            .any(|v| matches!(v, RendererViolation::TocNotObserved { .. })),
        "{violations:?}"
    );
}
...
fn renderer_contract_contains_asset_panics() {
    ...
    assert!(
        violations
            .iter()
            .any(|v| matches!(v, RendererViolation::AssetsPanicked)),
        "{violations:?}"
    );
}

#[test]
fn renderer_contract_reports_page_asset_collisions() {
    let models = pages(&corpus());
    ...
    assert!(
        collision
            .iter()
            .any(|v| matches!(v, RendererViolation::DuplicatePath { path } if path == "a.html")),
        "{collision:?}"
    );
}

#[test]
fn renderer_contract_reports_duplicate_assets() {
    let models = pages(&corpus());
...
#[test]
fn renderer_contract_reports_nondeterministic_assets() {
    let models = pages(&corpus());
    let violations = page_renderer(
        &ChangingAssetsRenderer {
            counter: Cell::new(0),
        },
        &models,
    );
    assert!(
        violations
            .iter()
            .any(|v| matches!(v, RendererViolation::AssetsNonDeterministic)),
        "{violations:?}"
    );
}
