...
impl Fixture {
    ...
    fn record(&self, dir: &Path, purpose: &str, args: &[&str], env: &[(&str, &str)], run: &Run) {
        // A record states what was observed. If git cannot answer, the
        // subject is unknown; it is never reported as a clean tree.
        let git = |argv: &[&str]| command_stdout(&root(), argv);
        let subject = match (
            git(&["git", "rev-parse", "HEAD"]),
            git(&["git", "status", "--porcelain"]),
        ) {
            (Ok(revision), Ok(status)) => {
                let dirty = if status.is_empty() { "no" } else { "yes" };
                format!("{revision} (working tree dirty: {dirty})")
            }
            _ => "unknown (git did not answer; working tree dirty: unknown)".to_owned(),
        };
        ...
        let text = format!(
            "experiment:   {id}\npurpose:      {purpose}\n\
             ...
             inherited:    the repository's .cargo/config.toml, rust-toolchain.toml and any\n\
             \x20             cargo configuration above them, because the fixture is built inside\n\
             \x20             the repository (ADR-0002 rule 6)\n\
...
fn discovery_is_per_crate_and_the_nearest_file_is_not_merged() {
    ...
    // Without this control, the silent runs below would read the same way if
    // the deny list never fired at all in this fixture.
    fixture
        .clippy(
            "2-control",
            "positive control: the same call in a core crate with the template",
            &["-p", "core-seeded"],
            &[],
        )
        .exits_nonzero()
...
fn a_lint_attribute_switches_the_deny_list_off() {
    ...
    // Control, as in experiment 2-control: the deny list fires in this
    // fixture when no attribute and no flag is in the way.
    fixture
        .clippy(
            "6-control",
            "positive control: the same call with no attribute and no flag",
            &["-p", "core-seeded"],
            &[],
        )
        .exits_nonzero()
