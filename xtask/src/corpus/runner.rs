...
#[must_use]
/// `stale` names the manifest digest the latest record graded when it is not
/// the current manifest's: every crate cell is then `not_run`, because a
/// result about an earlier registration says nothing about this one (P-A
/// stage 0; the CHG-004 unresolved list).
pub fn enforcement_map(
    ...
    if let Some(graded) = stale {
        let _ = writeln!(
            text,
            "**Stale.** The latest H4 record graded manifest `sha256:{graded}`, not the current one. No cell is claimed until the corpus is re-run.\n"
        );
    }
    for (id, description) in &manifest.cells {
        ...
        let status = if stale.is_some() {
            "not_run — the latest H4 record graded an earlier manifest; re-run `cargo xtask corpus run`"
        } else if failed {
            "review — downgraded"
