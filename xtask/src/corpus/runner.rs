    ...
    for collection in ["findings", "harness_edges"] {
        ...
        for finding in findings {
            ...
            if !finding["kind"].is_null() || finding["rule"] == "graph.cycle" {
                ...
                if witness["edge"].as_str().is_none_or(str::is_empty)
                    || witness["declared_in"].as_str().is_none_or(str::is_empty)
...
fn evidence_source(evidence: &str) -> String {
    if evidence.starts_with("no H4 ") {
        evidence.to_owned()
    } else {
...
fn mentions_cell(case: &Value, id: &str) -> bool {
    case["cells"]
        .as_array()
        .is_some_and(|cells| cells.contains(&json!(id)))
}

fn failed(case: &Value) -> bool {
    case["grade"]["passed"] != true
}

fn reference_case(case: &Value) -> bool {
    case["expected"] == "reference" || case["id"] == "X-M01"
}

fn supplementary_count(case: &Value) -> Option<u64> {
    [
        "/report/counts/cases",
        "/counts/cases",
        "/report/summary/counts/cases",
        "/summary/counts/cases",
    ]
    .into_iter()
    .find_map(|pointer| case.pointer(pointer).and_then(Value::as_u64))
}

fn module_case_label(case: &Value) -> String {
    let id = case["id"].as_str().unwrap_or("unnamed case");
    if reference_case(case) {
        format!("{id} (reference)")
    } else if id == "random-module-reference" {
        supplementary_count(case).map_or_else(
            || "supplementary random graphs".to_owned(),
            |count| format!("{count} supplementary cases"),
        )
    } else {
        id.to_owned()
    }
...
fn sorted_case_labels<'a>(cases: impl IntoIterator<Item = &'a Value>, module: bool) -> String {
    let mut labels = cases
        .into_iter()
        .map(|case| {
            if module {
                module_case_label(case)
            } else {
                case["id"].as_str().unwrap_or("unnamed case").to_owned()
            }
        })
        .collect::<Vec<_>>();
    labels.sort();
...
fn relevant<'a>(cases: &'a [Value], id: &str) -> Vec<&'a Value> {
    cases
        .iter()
        .filter(|case| mentions_cell(case, id))
        .collect()
}

/// Render crate and module projections independently.
#[must_use]
pub fn levels(
    manifest: &Manifest,
    crate_level: LevelEvidence<'_>,
    module_level: LevelEvidence<'_>,
) -> String {
    let module_ids: BTreeSet<String> = manifest
        .cases
        .iter()
        .filter(|case| case.level == Level::Module)
        .map(|case| case.id.clone())
        .collect();
    let classification_failed = crate_level
        .cases
        .iter()
        .any(|case| mentions_cell(case, "classification") && failed(case));
    let mut text = format!(
        "# Enforcement map\n\nGenerated by `cargo xtask docs` from the [public manifest](../{MANIFEST_PATH}), crate evidence source {}, and module evidence source {}. A corpus result is advisory; held-out acceptance is Kennedy's (DP-1.1b), and Accepted maturity is unchanged (DP-1.6).\n\n| Cell | Claim | Crate cases | Crate status | Module cases | Module status | Documented holes |\n| --- | --- | --- | --- | --- | --- | --- |\n",
        evidence_source(crate_level.evidence),
        evidence_source(module_level.evidence),
    );
    if crate_level.stale.is_some() || module_level.stale.is_some() {
        text.push_str("**Stale.** Evidence with an earlier manifest is not_run in its own level below; the other level is evaluated independently.\n\n");
    }
    for (id, description) in &manifest.cells {
        let crate_cases = relevant(crate_level.cases, id);
        let module_cases = relevant(module_level.cases, id);
        let crate_failed = classification_failed || crate_cases.iter().any(|case| failed(case));
        let module_headline = module_cases
            .iter()
            .filter(|case| {
                module_ids.contains(case["id"].as_str().unwrap_or("")) && !reference_case(case)
            })
            .copied()
            .collect::<Vec<_>>();
        let module_supplemental = module_cases
            .iter()
            .filter(|case| {
                !module_ids.contains(case["id"].as_str().unwrap_or("")) && !reference_case(case)
            })
            .copied()
            .collect::<Vec<_>>();
        let module_failed = module_headline.iter().any(|case| failed(case))
            || module_supplemental.iter().any(|case| failed(case));

        let crate_status = if let Some(graded) = crate_level.stale {
            format!(
                "not_run — the latest crate H4 record graded manifest sha256:{graded}; re-run `cargo xtask corpus run --level crate`"
            )
        } else if crate_failed {
            "review — downgraded".to_owned()
        } else if crate_cases.is_empty() {
            "not_run — no crate cases".to_owned()
        } else {
            "validated on registered crate cases only".to_owned()
        };
        let module_status = if let Some(graded) = module_level.stale {
            format!(
                "not_run — the latest module H4 record graded manifest sha256:{graded}; re-run `cargo xtask corpus run --level module`"
            )
        } else if module_failed {
            "review — downgraded".to_owned()
        } else if module_headline.is_empty() {
            if module_cases.iter().any(|case| reference_case(case)) {
                "not_run — reference-only observations; no accuracy claim".to_owned()
            } else {
                "not_run — no registered module cases".to_owned()
            }
        } else {
            "corpus validated (advisory; not accepted)".to_owned()
        };
        let mut holes = crate_cases
            .iter()
            .chain(module_cases.iter())
            .filter_map(|case| case["hole"].as_str())
            .map(str::to_owned)
            .collect::<Vec<_>>();
        holes.sort();
        holes.dedup();
        let _ = writeln!(
            ...
            markdown_cell(&sorted_case_labels(crate_cases, false)),
            markdown_cell(&crate_status),
            markdown_cell(&sorted_case_labels(module_cases, true)),
            markdown_cell(&module_status),
            markdown_cell(&holes.join("; ")),
        );
    }
    text.push_str(
...
/// Compatibility wrapper for the original crate-only projector.
#[must_use]
pub fn enforcement_map(
    manifest: &Manifest,
    cases: &[Value],
    evidence: &str,
    stale: Option<&str>,
) -> String {
    enforcement_map_levels(
        manifest,
        cases,
        evidence,
        stale,
        &[],
        "no H4 module record yet",
        None,
    )
}

#[must_use]
pub fn enforcement_map_levels(
    manifest: &Manifest,
    crate_cases: &[Value],
    crate_evidence: &str,
    crate_stale: Option<&str>,
    module_cases: &[Value],
    module_evidence: &str,
    module_stale: Option<&str>,
) -> String {
    levels(
        manifest,
        LevelEvidence {
            cases: crate_cases,
            evidence: crate_evidence,
            stale: crate_stale,
        },
        LevelEvidence {
            cases: module_cases,
...
pub fn run(root: &Path, args: &CorpusArgs) -> Result<u8> {
    ...
    if args.level == "module" {
        return super::module::run(root, args);
    }
    if args.level != "crate" {
        return Err(Error::new("unknown corpus level"));
    }
    ...
    let record = json!({
        ...
        "producer": {"name": "xtask corpus run", "version": env!("CARGO_PKG_VERSION"), "executable_sha256": crate::util::running_executable_sha256()?, "checker": expected_tool},
        ...
        "manifest": {"path": MANIFEST_PATH, "sha256": sha256_file(&root.join(MANIFEST_PATH))?, "amendments": amendments, "correction_commit": amendments.last().map_or(Value::Null, |a| a["commit"].clone())},
