...
- Before opening the PR, run `cargo xtask scope --task <id>`. It fails on any changed path, committed or not, that is outside the task record's `scope_globs`, outside the plan §4 layout, or on a protected surface the record's `protected_scope` does not name (plan §2.2 M4). It is a pre-PR command, not an L1 check (DP-5.2).
